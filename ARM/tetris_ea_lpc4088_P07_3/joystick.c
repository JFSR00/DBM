...
{
	if		 (!gpio_leer_pin(PUERTO2, (1 << P_ARR))){return JOYSTICK_ARRIBA;}
	else if(!gpio_leer_pin(PUERTO2, (1 << P_ABJ))){return JOYSTICK_ABAJO;}
	else if(!gpio_leer_pin(PUERTO2, (1 << P_IZQ))){return JOYSTICK_IZQUIERDA;}
	else if(!gpio_leer_pin(PUERTO2, (1 << P_DCH))){return JOYSTICK_DERECHA;}
	else if(!gpio_leer_pin(PUERTO2, (1 << P_CTR))){return JOYSTICK_CENTRO;}
...
int32_t leer_joystick(void)
{
	...
	if(!gpio_leer_pin(PUERTO2, (1 << P_CTR))){
		led_encender(LED1);
		led_encender(LED2);
		led_encender(LED3);
		led_encender(LED4);
