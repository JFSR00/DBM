...
int main(void)
{
    ...
    glcd_inicializar();
		...
		/* Borrar peticiones de interrupción del timer 0 y GPIO pendientes.
     */
    ...
    /* Asignar prioridad 1 a la interrupción del timer 0, y prioridad 0 a GPIO.
     */
		NVIC_SetPriority(GPIO_IRQn, 0);
		NVIC_SetPriority(TIMER0_IRQn, 1);

    /* Habilitar la interrupción del timer 0 y de GPIO.
     */
...
void TIMER0_IRQHandler(void)
{
		...
		/* Limpiamos los registros de interrupción
		 */
		TIMER0->IR = 1;
		NVIC_ClearPendingIRQ(TIMER0_IRQn);
